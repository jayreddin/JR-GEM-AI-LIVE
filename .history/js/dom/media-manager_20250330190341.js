...
export class MediaManager {
    ...
    setupEventListeners() {
        ...
        this.audioChatBtn?.addEventListener('click', () => this.toggleAudioChat());
        this.flipCameraBtn?.addEventListener('click', () => this.flipCamera());

        // Listen for transcription events
        document.addEventListener('transcription', (event) => {
            if (event.detail && event.detail.text) {
                // Only auto-send in audio chat mode
                if (this.isAudioChatMode && event.detail.isFinal) {
        ...
        // Listen for AI response end to auto-activate mic in audio chat mode
        document.addEventListener('aiResponseEnd', () => {
            if (this.isAudioChatMode && !this.isMicActive) {
                // Short delay before activating microphone to avoid feedback
                setTimeout(() => this.toggleMic(), 500);
            }
        });
    }
    ...
    async toggleMic() {
        try {
            if (!this.isMicActive) {
                ...
                // Start recording with speech recognition enabled and auto-end if in audio chat mode
                await this.audioRecorder.start(null, true, this.isAudioChatMode);
                
                // Listen for silence detection if in audio chat mode
                if (this.isAudioChatMode) {
                    this.audioRecorder.addEventListener('silence_detected', () => {
                        // Only process if we're still in audio chat mode and mic is active
                        if (this.isAudioChatMode && this.isMicActive) {
                            this.toggleMic(); // Automatically deactivate mic on silence
                        }
                    });
                }
                ...
                // Different status message for audio chat mode
                if (this.isAudioChatMode) {
                    StatusManager.addStatus('Listening... (speak and pause to send)', 3000);
                } else {
    ...
    async toggleAudioChat() {
        this.isAudioChatMode = !this.isAudioChatMode;
        this.audioChatBtn?.classList.toggle('active', this.isAudioChatMode);
        localStorage.setItem('audioChatMode', this.isAudioChatMode);

        if (this.isAudioChatMode) {
            StatusManager.addStatus('Audio chat mode activated', 3000);
        } else {
            StatusManager.addStatus('Audio chat mode deactivated', 3000);
        }
    }
