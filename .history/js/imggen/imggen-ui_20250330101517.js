...
export class ImageGenUI {
    constructor() {
        ...
        this.setupUI();
        this.initializeImageDisplay(); // Ensure image display is initialized
    }

    initializeImageDisplay() {
        if (!this.imageDisplay?.querySelector('.image-display')) {
            console.log("Creating .image-display element");
            const imageDisplayDiv = document.createElement('div');
            imageDisplayDiv.className = 'image-display';
            imageDisplayDiv.innerHTML = '<div class="placeholder-text">Generated images will appear here</div>';
            // Insert image-display right after the model display
            const modelDisplay = this.imageDisplay?.querySelector('.img-gen-model-display');
            if (modelDisplay) {
                modelDisplay.insertAdjacentElement('afterend', imageDisplayDiv);
            } else {
                this.imageDisplay?.appendChild(imageDisplayDiv);
            }
        }
