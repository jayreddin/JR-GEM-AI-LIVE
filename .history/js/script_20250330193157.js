...
async function ensureAgentReady() {
    ...
    if (!agent.initialized) {
        StatusManager.addStatus("Initializing model...", 2000);
        try {
            await agent.initialize();
            StatusManager.addStatus("Model ready", 2000);
            return true;
        } catch (error) {
            console.error('Error initializing agent:', error);
            StatusManager.addStatus(`Initialization failed: ${error.message || 'Unknown error'}`, 5000);
            
            // Try to reconnect on initialization failure
            try {
                await agent.disconnect();
                await new Promise(resolve => setTimeout(resolve, 1000));
                return await ensureAgentReady(); // Try again once
            } catch (e) {
