 ...
 * @param {boolean} withAudio - Whether to initialize audio components or not
 * @returns {Promise<boolean>} True if ready, false otherwise.
 */
async function ensureAgentReady(withAudio = false) {
    if (!agent) {
        console.error("Agent not initialized.");
        StatusManager.addStatus("Agent not ready. Please refresh the page.", 3000);
        return false;
    }
    ...
    if (!agent.connected) {
        StatusManager.addStatus("Connecting to model...", 2000);
        try {
            ...
            // First try to establish connection with timeout
            ...
            // Only initialize audio if specifically requested
            if (withAudio) {
                // Initialize the agent with audio components
            ...
            StatusManager.addStatus("Connected to model", 2000);
            
            // Verify connection is stable by checking state
            if (!agent.connected) {
            ...
            StatusManager.addStatus(`Connection failed: ${error.message || 'Unknown error'}. Please try again.`, 5000);
            
            // Cleanup any partial connection state
            try {
                await agent.disconnect();
            } catch (e) {
                console.warn("Could not properly disconnect:", e);
            }
    ...
    // Only initialize audio when explicitly requested
    if (withAudio && !agent.initialized) {
        StatusManager.addStatus("Initializing audio components...", 2000);
        try {
            await agent.initialize();
            StatusManager.addStatus("Audio components ready", 2000);
            return true;
        } catch (error) {
            console.error('Error initializing audio components:', error);
            StatusManager.addStatus(`Audio initialization failed: ${error.message || 'Unknown error'}`, 5000);
            return false;
        }
    }
...
function setupUIEventListeners() {
    ...
    cameraBtn?.addEventListener('click', async () => {
        if (!await ensureAgentReady(true)) return; // Initialize with audio components for media
    ...
    micBtn?.addEventListener('click', async () => {
        if (!await ensureAgentReady(true)) return; // Initialize with audio components for mic
    ...
    audioChatBtn?.addEventListener('click', async () => {
        if (!await ensureAgentReady(true)) return; // Initialize with audio components
